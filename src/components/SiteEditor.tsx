...
export function SiteEditor() {
  ...
  async function addGalleryImage(file: File) {
    try {
      const updated = await api.addGalleryImage(file);
      setConfig(updated);
  ...
  return (
    <div className="site-editor">
      ...
      <div className="card editor-card">
        ...
        <div className="image-upload-row">
          <div className="image-preview">
            {form.logo ? <img src={form.logo} alt="logo" /> : <div className="image-placeholder">لا يوجد شعار</div>}
          </div>
          <div className="upload-controls">
            <input ref={logoInputRef} type="file" accept="image/*" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadImage(f, 'logo'); }} />
            <button className="btn btn-ghost" onClick={() => logoInputRef.current?.click()}>رفع شعار</button>
      ...
      <div className="card editor-card">
        ...
        <div className="image-upload-row">
          <div className="image-preview image-preview-wide">
            {form.heroImage ? <img src={form.heroImage} alt="hero" /> : <div className="image-placeholder">لا توجد صورة خلفية</div>}
          </div>
          <div className="upload-controls">
            <input ref={heroInputRef} type="file" accept="image/*" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadImage(f, 'heroImage'); }} />
            <button className="btn btn-ghost" onClick={() => heroInputRef.current?.click()}>رفع صورة الخلفية</button>
      ...
      <div className="card editor-card">
        ...
        <div className="form-field">
          <label className="checkbox-label">
            <input type="checkbox" checked={form.ctaPulse ?? true} onChange={(e) => setForm({ ...form, ctaPulse: e.target.checked })} />
            تفعيل النبض على الزر
          </label>
        </div>
      </div>
      ...
      <div className="card editor-card">
        ...
        <div className="form-field">
          <label>العنوان الفرعي للقسم</label>
      ...
      <div className="card editor-card">
        ...
        <div className="gallery-editor-grid">
          ...
          <div className="gallery-editor-add">
            <input ref={galleryInputRef} type="file" accept="image/*" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) addGalleryImage(f); }} />
            <button className="btn btn-ghost" onClick={() => galleryInputRef.current?.click()}>+ إضافة صورة</button>
          </div>
        </div>
      </div>
