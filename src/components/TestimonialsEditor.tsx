...
interface Props { items: Testimonial[]; onChanged: () => void; }
...
export function TestimonialsEditor({ items, onChanged }: Props) {
  ...
  async function handleDelete() {
    ...
    try { await db.deleteTestimonial(confirmDelete.id); showToast('تم الحذف', 'success'); setConfirmDelete(null); onChanged(); }
    catch (e) { showToast(e instanceof Error ? e.message : 'فشل', 'error'); }
  }

  return (
    <div className="admin-section">
      <div className="list-toolbar"><button className="btn btn-primary" onClick={() => { setEditing(null); setShowForm(true); }}>+ إضافة رأي</button></div>
...
function TestimonialForm({ item, onDone }: { item: Testimonial | null; onDone: () => void }) {
  const [form, setForm] = useState({ name: item?.name ?? '', role: item?.role ?? '', text: item?.text ?? '', rating: item?.rating ?? 5, avatar: item?.avatar ?? '', sort_order: item?.sort_order ?? 0 });
  ...
  return (
    <form onSubmit={submit} className="form-grid">
      ...
      <div className="form-field"><label>الصفة</label><input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div>
      <div className="form-field"><label>التقييم (1-5)</label><input type="number" min={1} max={5} value={form.rating} onChange={(e) => setForm({ ...form, rating: Number(e.target.value) })} /></div>
      ...
      <div className="form-field form-field-full"><label>رابط صورة العميل</label><input value={form.avatar} onChange={(e) => setForm({ ...form, avatar: e.target.value })} />{form.avatar && <img src={form.avatar} alt="معاينة" className="image-preview avatar-preview" />}</div>
      <div className="form-field form-field-full"><label>النص *</label><textarea required rows={3} value={form.text} onChange={(e) => setForm({ ...form, text: e.target.value })} /></div>
