...
export function MaterialsManager() {
  ...
  async function load() {
    setLoading(true);
    try { setMaterials(await api.getMaterials({ category: filterCat || undefined, lowStock: lowStockOnly || undefined })); }
    catch { /* ignore */ }
  ...
  return (
    <div className="mgr-section">
      <div className="toolbar">
        ...
        <label className="check-label"><input type="checkbox" checked={lowStockOnly} onChange={(e) => setLowStockOnly(e.target.checked)} /> مخزون منخفض</label>
      </div>
      {materials.length === 0 ? (
        ...
        <div className="table-card">
          <table className="data-table">
            <thead><tr><th>الاسم</th><th>التصنيف</th><th>المخزون</th><th>الوحدة</th><th>السعر</th><th>المورد</th><th></th></tr></thead>
            <tbody>
              {materials.map((m) => {
                const low = m.stock <= m.lowStockThreshold;
                return (
                  <tr key={m._id}>
                    <td className="td-strong">{m.name}</td>
                    <td>{MATERIAL_CATEGORY_LABELS[m.category]}</td>
                    <td className={low ? 'td-danger' : ''}>{m.stock} {low && '⚠'}</td>
                    <td>{m.unit}</td>
...
function MaterialForm({ material, onDone }: { material: Material | null; onDone: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="form-grid">
      ...
      <div className="field"><label>الوحدة</label><input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} /></div>
      <div className="field"><label>المخزون</label><input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: Number(e.target.value) })} /></div>
      <div className="field"><label>حد المنخفض</label><input type="number" value={form.lowStockThreshold} onChange={(e) => setForm({ ...form, lowStockThreshold: Number(e.target.value) })} /></div>
      <div className="field"><label>السعر</label><input type="number" value={form.pricePerUnit} onChange={(e) => setForm({ ...form, pricePerUnit: Number(e.target.value) })} /></div>
      <div className="field field-full"><label>المورد</label><input value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} /></div>
