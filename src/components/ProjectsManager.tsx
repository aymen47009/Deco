...
export function ProjectsManager({ projects, loading, onChanged }: Props) {
  ...
  async function handleDelete() {
    ...
    try {
      await db.deleteProject(confirmDelete.id);
      ...
      {projects.length === 0 ? (
        ...
        <div className="table-wrap">
          <table className="data-table">
            <thead>
              <tr><th>الكود</th><th>الاسم</th><th>الهاتف</th><th>نوع العمل</th><th>المساحة</th><th>الحالة</th><th>التاريخ</th><th>إجراءات</th></tr>
            </thead>
            <tbody>
              {projects.map((p) => (
                <tr key={p.id}>
