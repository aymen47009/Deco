...
import {
  ...
  type ProjectMaterialInput, type ArtisanSummary,
} from '../types';
...
function ProjectDetailModal({ project, onClose, onChanged }: { project: Project; onClose: () => void; onChanged: () => void }) {
  ...
  const [selectedArtisanId, setSelectedArtisanId] = useState(project.assignedArtisanId || '');
  ...
  async function loadDetail() {
    try {
      ...
      setDetail(d);
      setSelectedArtisanId(d.assignedArtisanId || '');
    } catch { /* ignore */ }
  }

  useEffect(() => {
    loadDetail();
    api.getArtisans().then(setArtisans).catch(() => setArtisans([]));
  }, []);

  async function assignArtisan() {
    if (!selectedArtisanId) {
      showToast('اختر حرفياً أولاً', 'error');
      return;
    }
    try {
      await api.assignProjectArtisan(project._id, selectedArtisanId);
      showToast('تم تعيين الحرفي', 'success');
      loadDetail();
      onChanged();
    } catch (e) {
      showToast(e instanceof Error ? e.message : 'فشل', 'error');
    }
  }
  ...
  return (
    <Modal open={true} title={`مشروع ${p.code} — ${p.customer.name}`} onClose={onClose} size="lg">
      ...
      <div className="pdt-body">
          ...
          <div className="pdt-overview">
            ...
            {fin && (
              <div className="pdt-fin-grid">
                ...
                <FinCard label="إجمالي مدفوعات مؤكدة" value={fin.totalVerifiedPayments} color="var(--ok)" />
                <FinCard label="صافي الربح" value={fin.netProfit} color="var(--brand)" highlight />
              </div>
            )}
            <div className="pdt-edit-amount">
              <label>تعيين حرفي للمشروع</label>
              <div className="pdt-link-row">
                <select className="status-select" value={selectedArtisanId} onChange={(e) => setSelectedArtisanId(e.target.value)}>
                  <option value="">اختر الحرفي</option>
                  {artisans.map((artisan) => (
                    <option key={artisan._id} value={artisan._id}>
                      {artisan.name} - {artisan.phone}
                    </option>
                  ))}
                </select>
                <button className="btn btn-primary btn-sm" onClick={assignArtisan}>تعيين</button>
              </div>
              <div style={{ marginTop: '10px' }}>
                {p.assignedArtisanId ? <span className="pdt-badge pdt-badge-ok">تم التعيين</span> : <span className="pdt-badge pdt-badge-pending">غير معين</span>}
              </div>
            </div>
            <div className="pdt-edit-amount">
        ...
        {tab === 'artisan' && (
          <ArtisanTab project={p} onChanged={loadDetail} />
        )}
...
function ArtisanTab({ project, onChanged }: { project: Project; onChanged: () => void }) {
