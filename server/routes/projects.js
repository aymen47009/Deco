...
router.delete('/:id/workers/:workerId', async (req, res) => {
  try {
    const project = await Project.findById(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found' });
    project.assignedWorkers = project.assignedWorkers.filter((w) => w.toString() !== req.params.workerId);
    await project.save();
    await Worker.findByIdAndUpdate(req.params.workerId, { $pull: { assignedProjects: project._id }, status: 'available' });
    res.json(project);
  } catch (err) { res.status(400).json({ error: err.message }); }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (!project) return res.status(404).json({ error: 'Project not found' });
    await Worker.updateMany({ _id: { $in: project.assignedWorkers } }, { $pull: { assignedProjects: project._id }, status: 'available' });
