...
app.get("/api/health", (_req, res) => {
  res.json({ status: "ok", db: isDBConnected() ? "connected" : "disconnected", timestamp: new Date().toISOString() });
});
...
connectDB().then(() => {
  app.listen(PORT, () => { console.log(`[server] Deco Workshops API running on http://localhost:${PORT}`); });
